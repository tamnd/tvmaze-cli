// Package cli assembles the tvmaze command tree from the tvmaze
...
// NewApp assembles the kit application from the tvmaze domain. The
...
//
// To add a command, declare it in tvmaze/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := tvmaze.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(tvmaze.Domain{}).Register(app)
