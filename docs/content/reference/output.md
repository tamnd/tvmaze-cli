...
describes what users get. Pick a format with `-o`, or let tvmaze choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
tvmaze <command> -o table   # aligned columns for reading
tvmaze <command> -o jsonl   # one JSON object per line, for piping
tvmaze <command> -o json    # a single JSON array
tvmaze <command> -o csv     # spreadsheet friendly
tvmaze <command> -o tsv     # tab-separated
tvmaze <command> -o url     # just the URL column
tvmaze <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
tvmaze <command> --fields id,title,url
```
...
```bash
tvmaze <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
tvmaze <command>            # a table, because this is a terminal
tvmaze <command> | wc -l    # JSONL, because this is a pipe
```
